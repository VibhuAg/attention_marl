...
def concatenate(obs, comm, ablate_kwargs=None):
    ...
    # print(agent, obs[agent].shape, obs[agent])
    return obs
...
def make_advcomm_env(adv_agents, good_policy_dir, victim, ac_kwargs, ablate_kwargs, 
    ...
    #attn = MultiheadAttention(good_comm_space.shape[0], 1)
    if good_policy_dir:
...
def ppo(env_fn=None, actor_critic=MLPActorCritic, ac_kwargs=dict(), seed=0, 
    ...
    def compute_loss_pi(data):
        ...
        # print(attn_outputs)
        obs = o_tensor + attn_outputs
        obs = ac.normalize(obs)
        pi, logp = ac.pi(obs, act)

        #attn_outputs, attn_output_weights = attn(o_tensor, o_tensor, o_tensor)
    ...
    def update(data):
        ...
        for i in range(train_pi_iters):
            ...
            # pi_outputs = ac.pi.logits_net(data['obs'])
            kl = pi_info['kl']
            ...
            # else:
            #     print("Backpropagation is not being performed: pi")
            
            # if any(param.grad is not None for param in ac.attn.parameters()):
    ...
    for epoch in range(epochs):
        ...
        # want to feed o into the multhead attention
        ...
        """
            Multiple multihead attentions for each agent or one multihead attention for all agents?
            Maybe make a class containing multihead attention, some linear layers and the policy network
            and then call the optimizer on an object of that class
        """
        for t in range(steps_per_epoch):
            good_actions, values, log_probs = {},{},{}
            for agent in good_agent_name:
                a, v, logp = ac.step(torch.from_numpy(o[agent]).to(Param.device).type(Param.dtype), train=obs_normalize)
                values[agent] = v
...
if __name__ == '__main__':
    ...
    ppo(lambda:env, actor_critic=MLPActorCritic,
        ac_kwargs=dict(hidden_sizes=[args.hid]*args.l, beta=args.beta, 
                       recurrent=args.recurrent, ep_len=args.max_cycle, n_pursuers=args.n_pursuers), 
